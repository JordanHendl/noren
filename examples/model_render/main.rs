//! Run with `cargo run --example model_render [primitive]` to load a default
//! primitive's geometry from the database and render it to the display.
...
fn run() -> Result<(), Box<dyn Error>> {
    ...
    let mut geometry_db = db.geometry_mut();

    let host_geometry = geometry_db.fetch_raw_geometry(&geometry_entry)?;
    println!(
    ...
    let device_geometry = geometry_db.fetch_gpu_geometry(&geometry_entry)?;
    println!(
    ...
    blit_image_to_display(&mut ctx, framebuffer, FRAMEBUFFER_DIM, "model_render")?;
...
fn render_geometry(
    ...
    ring.record(|list| {
        ...
        let viewport = Viewport {
            area: FRect2D {
                w: FRAMEBUFFER_DIM[0] as f32,
                h: FRAMEBUFFER_DIM[1] as f32,
                ..Default::default()
            },
            scissor: Rect2D {
                w: FRAMEBUFFER_DIM[0],
